...
- [x] T001 Amend constitution in `.specify/memory/constitution.md`: (a) Principle I: change "7 projects" to "variable project count with cluster support", change "Star positions are 7 hard-coded (x, y, z) values" to "Star and cluster positions are data-driven from the project array", add `status`, `isCluster`, `clusterMembers` to approved data model fields, REMOVE `glyphName`, `glyphRotation`, `glyphType`, `glyphAtlasIndex` from the data model field list; (b) Principle II: change "7 project star sprites" to "variable-count project star sprites and cluster groups"; (c) Principle V: change "7 project colors" to "per-project accent colors"; (d) bump version to 1.4.0
- [x] T002 [P] Remove `GLYPH_ATLAS_CELLS` export array (lines 9-74) from `js/data.js` and remove `glyphName`, `glyphRotation`, `glyphType`, `glyphAtlasIndex` fields from all 7 PROJECTS entries in `js/data.js`
- [x] T003 [P] Remove unused MSDF `fragmentShader` export and `GLYPH_ATLAS_CELLS` import from `js/glyph-compositor.js` — keep `init()`, `setHoveredProject()`, `clearHover()`, `getNavRect()` functions intact
...
- [x] T004 Add `status: "active"`, `isCluster: false`, `clusterMembers: null` fields to all existing 7 PROJECTS entries in `js/data.js`
...
- [x] T011 Add `SOCIAL_LINKS` export array to `js/data.js` with 11 entries per data-model.md: LinkedIn, Facebook, X, GitHub, NPM, PyPI, Docker Hub, VS Marketplace, Codecov, Medium, Gravatar — each with platform name and URL

- [x] T011a Update `js/terminal.js`: (a) change hardcoded "7 Essentials Active" string (lines 22, 90) to dynamically compute active count from PROJECTS: `PROJECTS.filter(p => p.status === 'active' && !p.isCluster).length + ' Essentials Active'`, (b) replace the hardcoded 7-element `percentages` array (line 47) with dynamically computed values: `Array.from({length: PROJECTS.length}, (_, i) => Math.round(((i + 1) / PROJECTS.length) * 100))`, (c) decide whether clusters count in the terminal scan display (recommend: exclude clusters, count only individual interactive stars)
- [x] T011b [P] Update sr-only descriptive paragraph in `index.html` (line 178): change "An interactive starfield with 7 glowing star nodes, each representing a portfolio project" to reflect the new count (9 individual stars + 2 clusters = 11 visual elements) and mention cluster concept
...
- [x] T028 [US2] Refactor `js/constellation-lines.js` init() to create SVG `<defs>` element inside svgContainer. Use a LOOP over zone indices (not 3 separate blocks) to keep line count minimal. Each zone gets: (a) a `<filter>` (id="zone-glow-N") with `color-interpolation-filters="sRGB"` and expanded filter region (`x="-50%" y="-50%" width="200%" height="200%"`) containing 4 primitives: `feGaussianBlur` (stdDeviation 3, result="blur") → `feFlood` (flood-color=zone hex, flood-opacity 0.6, result="color") → `feComposite` (in="color" in2="blur" operator="in", result="coloredGlow") → `feMerge` (coloredGlow + SourceGraphic); (b) a `<linearGradient>` (id="zone-grad-N") with `gradientUnits="objectBoundingBox"` using 3 stops: zone color at 100% opacity (0%), zone color brightened +20% lightness (50%), zone color at 40% opacity (100%). Zone hex colors from nebulaHueRgb: Zone 0 #6B40A1, Zone 1 #B8870A, Zone 2 #1A9E8F.
- [x] T029 [US2] Add `initWatermarkLines()` function to `js/constellation-lines.js`. Called once after reveal completes (via 'reveal-complete' event or explicit call from app.js). Creates a `<g class="watermark-lines">` group containing faint dashed lines (stroke-dasharray "8 12", stroke-opacity 0.20 — NOT 0.15 which is invisible on consumer monitors against the #0D0B09 background, zone-colored but lightened and desaturated for "antique star chart" feel) connecting all members of all 3 zones using chain topology (N-1 lines for N members). EXCLUDE dead-rock-cluster from line connections per T022. Store watermark line references in a `watermarkLines` array (separate from `activeLines`). Each watermark line entry: {element, star1, star2, zoneIndex}. Merge watermarkLines and activeLines into a single `allTrackedLines` array for tick() to iterate once (saves duplicate iteration).
- [x] T030 [US2] Update `tick()` in `js/constellation-lines.js` to update positions of ALL tracked lines per frame using the unified `allTrackedLines` array (populated by T029 and T033). Single iteration: get star world positions via getWorldPosition + project3DtoScreen, update SVG line x1/y1/x2/y2 attributes. For cluster endpoints, `THREE.Group.getWorldPosition()` works identically to `Sprite.getWorldPosition()` — returns the group center position.
- [x] T031 [US2] Refactor `createZoneLines()` in `js/constellation-lines.js` to create "active" lines with premium visual treatment: each line gets `stroke="url(#zone-grad-N)"`, `filter="url(#zone-glow-N)"`, `stroke-width: 2`, `stroke-linecap: round`, `stroke-opacity: 0.7`. Active lines are created in a `<g class="active-lines">` group overlaid on the watermark layer.
- [x] T032 [US2] Add energy flow animation to active lines in `js/constellation-lines.js`. After draw-on animation completes, apply a repeating GSAP tween: `gsap.to(line, { attr: { 'stroke-dashoffset': -cycleLength }, duration: totalLength / 200, repeat: -1, ease: 'none' })` — duration is length-proportional (~200px/s travel speed), not a fixed 2s, so short and long lines flow at the same perceived speed. Use dasharray `"15 10 5 10"` (complex pattern: long dash, gap, short dash, gap — creates "morse code energy pulse" rhythm). Replace the existing `updatePulse()` breathing opacity on active lines with energy flow — the two effects compete for attention. Skip under `prefers-reduced-motion` — show solid lines instead.
- [x] T033 [US2] Refactor `onZoneChange()` in `js/constellation-lines.js` to handle watermark↔active transitions. When a zone activates: create active lines for that zone (overlaying the watermark). When a zone deactivates: fade out and remove active lines (watermark remains). The `fadeSequence` guard must cover both active line creation and removal to prevent stale callbacks during rapid scrolling.
- [x] T034 [US2] Update panel-open/panel-close handlers in `js/constellation-lines.js` to hide BOTH svgContainer layers (watermark + active) on panel-open, restore on panel-close. Since both layers are children of the same svgContainer, setting `svgContainer.style.display = 'none'` already handles this — verify it does.
- [x] T035 [US2] Add reduced-motion fallback in `js/constellation-lines.js`: when `prefers-reduced-motion` is true, watermark lines show as SOLID (remove stroke-dasharray entirely — dashed patterns can cause visual vibration for vestibular disorders), active lines show as solid at full opacity (no energy flow, no glow filter, no draw-on animation). Zone transitions are instant (no crossfade). Keep watermark lines VISIBLE under reduced-motion — they serve an information purpose (showing constellation structure). Ensure both the old `updatePulse()` AND the new energy flow are gated.
- [x] T036 [US2] Verify constellation lines track correctly on window resize — `tick()` uses `project3DtoScreen()` which reads camera and renderer dimensions, so SVG endpoints should auto-update. Verify watermark lines also update. Test by resizing browser window while lines are visible.
...
- [x] T041 [P] [US5] Add social links HTML section to `#status-panel` in `index.html`, below the existing `.status-readout` div. Create a `<div class="social-links">` containing 11 `<a>` elements, each with: `href` from SOCIAL_LINKS data, `target="_blank"`, `rel="noopener noreferrer"`, `aria-label="[Platform name]"`, and an inline SVG icon (`<svg viewBox="0 0 24 24" width="18" height="18"><path d="..." fill="currentColor"/></svg>`). SVG path data: use simplified brand icons for each platform (LinkedIn, Facebook, X, GitHub, NPM, PyPI, Docker Hub, VS Marketplace, Codecov, Medium, Gravatar). Each icon under 500 bytes.
- [x] T042 [P] [US5] Style social links section in `css/styles.css`. Rules: `.social-links` flex-wrap row with gap 8px, padding-top 12px, border-top 1px solid var(--color-brass-dark). Links: `color: var(--color-brass-light, #C8A84B)`, hover: `color: #ffd700`, transition 0.2s. Focus indicator: golden glow outline per constitution Principle III (3px outline + box-shadow with dark offset ring). Hide on mobile `@media (max-width: 767px) { .social-links { display: none; } }`.
- [x] T043 [US5] Add social links to mobile hamburger menu in `js/interactions.js` or `index.html`. On mobile (<768px), social links should appear at the bottom of the hamburger nav panel as a row of icon links. Ensure they are keyboard-reachable when hamburger is open.
- [x] T044 [US5] Verify all 11 social link URLs resolve correctly by checking each `href` against the SOCIAL_LINKS data in `js/data.js`. Verify `rel="noopener noreferrer"` on all links. Verify `aria-label` includes platform name for screen readers.
...
- [x] T045 Verify complete reduced-motion compliance across ALL new features: energy flow animation disabled, constellation intro showcase skipped, cluster pulse disabled, zone transitions instant, watermark lines static solid. Test by enabling `prefers-reduced-motion: reduce` in browser devtools and walking through all interactions in `js/constellation-lines.js`, `js/scroll-zones.js`, `js/textures.js`, `js/animations.js`.
- [x] T046 Verify high-contrast mode (`prefers-contrast: more`) across all new features: decorative elements (watermark lines, cluster halos, social link icons) either hidden or have sufficient contrast. Test in browser devtools in `css/styles.css` and `js/sidebar-hieroglyphs.js`.
- [x] T047 Cross-browser testing: load page in Chrome, Firefox, and Safari. Verify (a) SVG filters render correctly (feGaussianBlur glow), (b) constellation lines track star positions accurately, (c) energy flow animation runs smoothly, (d) all panels open correctly, (e) social link icons display. Document any browser-specific issues.
- [x] T048 Performance verification: use browser DevTools to confirm (a) draw calls < 30 steady state (Performance tab → GPU → Draw calls), (b) 60fps maintained with all watermark lines visible on Intel Iris-class GPU, (c) page weight increase < 15KB from new data + SVG icons, (d) no memory leaks from constellation line creation/destruction during repeated zone scrolling.
- [x] T049 Run complete quickstart.md testing checklist in `specs/009-constellation-zone-enhancements/quickstart.md` — verify all 24 items pass.
- [x] T050 Update `CONSTELLATIONS.md` in project root to reflect the new state: update zone membership table, project list, constellation line behavior description, and mark "NEW DATA" section as incorporated.
